...
> **Статус:** модель побудована й **перевірена**. Вона відтворює всі три
> parity-фікстури (`parity/trajectories/*.json`) покадрово — 24 кадри × 2 бійці,
> **0 розбіжностей** за полями `task`, `frame_offset`, `hp`, `stamina`.
>
> Файли: `rddl/duel_domain.rddl`, `rddl/duel_instance.rddl`, `rddl/check_parity.py`.
...
- `observ-fluent` для 7-компонентного вектора спостережень — модель наразі
  повністю спостережувана.
- Суперник як політика в рівняннях (зараз `@b` керується ззовні, що потрібно для
  програвання фікстур, але не для навчання).
- Порівняння PPO з оптимальною політикою від планувальника.
